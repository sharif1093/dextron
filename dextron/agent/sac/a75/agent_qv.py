...
class Agent(AgentSchedule):
    def calculate_loss(self, state, action, reward, next_state, masks):
        with KeepTime("loss"):
            ...
            expected_q_value = self.policy.model["softq"](state_repr_softq, action)
            ...
            new_action, log_prob, z, mean, log_std = self.policy.evaluate_actions(state_repr_softq.detach())
            ...
            expected_new_q_value = self.policy.model["softq"](state_repr_softq.detach(), new_action)
